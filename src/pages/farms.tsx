...
import { getLpPrice, getMMTNPriceFromLP } from '@/application/staking/getSignMessage'
  ...
  return (
    <>
      <div className="text-center justify-center mb-2">
        {/* <h6 className="text-2xl bg-min">Don't Deposit, Launch date will be announced</h6>
        <br /> */}
  ...
  return (
    <CyberpunkStyleCardForAirdrop
      ...
      {/* {!isMobile && (
        <Row
          type="grid-x"
          className="mb-3 h-12  sticky -top-6 backdrop-filter z-10 backdrop-blur-md bg-[rgba(20,27,35,0.2)] mr-scrollbar rounded-xl mobile:rounded-lg gap-2 grid-cols-[auto,1.5fr,1.2fr,1fr,1fr,auto]">
...
function StakingCardCollapseItemFace({ open, info }: { open: boolean; info: IStakeInfo }) {
  ...
  useEffect(() => {
    async function fetchTokenPrice(tokenMint: string) {
      let resp;
      try {
        if (tokenMint == MMTNTokenAddress) {
          resp = await getMMTNPriceFromLP(MMTNLpAddress)
    ...
    >
      <Row className="p-6 mobile:py-3 mobile:px-4 flex-grow ring-inset ring-1.5 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-3xl mobile:rounded-xl items-center gap-3">
        <div className="flex-grow">
          <div className="text-gray-600 font-medium text-sm mobile:text-2xs mb-1">Deposited</div>
          ...
          <div className="text-gray-600 font-medium text-sm mobile:text-xs">
...
function TextInfoItem({
  ...
  return (
    <Col className={twMerge('w-max', className)}>
      <div className="mb-1 text-gray-600 font-medium text-sm mobile:text-2xs">{name}</div>
      <Col className="flex-grow justify-center">
        <div className="text-base mobile:text-xs">{value || '--'}</div>
        {subValue && <div className="text-sm mobile:text-2xs text-gray-600">{subValue}</div>}
      </Col>
    </Col>
  )
}
