...
export default function Swap() {
  return (
    <SwapUIContextProvider>
      <SwapEffect />
      <PageLayout mobileBarTitle="Swap" metaTitle="Swap - Magic Mittens">
        <LogoAndBackground />
    ...
    useNotification.getState().popConfirm({
      ...
      description: (
        <div className="space-y-4  text-center">
          <p className="text-[#7E730A] font-bold">Confirm the token mint address is correct before swapping!</p>
          <p>
            Many tokens have <span className="text-[#7E730A]">similar or identical</span> token tickers and icons.
          </p>
          <p>
            By clicking below, you understand that you are fully responsible for confirming the token you are trading.
          </p>

          <Row className="justify-center items-center gap-2 my-4 bg-tooltip rounded py-3 w-full">
...
function SwapHead() {
  return (
    <Row className="justify-center mb-4 mobile:mb-2">
      <Tabs
        currentValue={'Swap'}
    ...
    >
      ...
      <div className="space-y-5 mt-5 mobile:mt-0">
        ...
        <div className="relative h-8">
          <Row
            className={`absolute items-center transition-all ${executionPrice ? 'left-4' : 'left-1/2 -translate-x-1/2 -translate-y-2'
              }`}
          >
            <Icon
              size="sm"
  ...
  return (
    <FadeIn>
      {bothHaveAmount && !hasAcceptedPriceChange && !isApprovePanelShown && (
        <Row className="mt-5 bg-tooltip rounded-xl py-2 px-6 mobile:px-4 items-center justify-between">
  ...
  return (
    <FadeIn>
      {solBalance && lt(solBalance, SOL_BASE_BALANCE) && gte(solBalance, 0) && (
        <Row className="text-sm mt-4 text-[#FF1111] items-center justify-center">
  ...
  return (
    <Col>
      ...
      <FadeIn>
        {priceImpact ? (
          <div
            className={`font-medium text-xs whitespace-nowrap ${isDangerousPrice ? 'text-[#DA2EEF]' : isWarningPrice ? 'text-[#FF1111]' : 'text-color'
              } transition-colors`}
          >
    ...
    >
      ...
      {maxSpent ? (
        ...
        <SwapCardItem
          ...
          fieldValue={`${toString(minReceived, { decimalLength: `auto ${downCoin?.decimals}` })} ${downCoin?.symbol ?? '--'
            }`}
      ...
      <SwapCardItem
        ...
        fieldValueTextColor={isDangerousPrice ? '#DA2EEF' : isWarningPrice ? '#D8CB39' : '#D0D0D8'}
      ...
      {canShowKline && (
        <div className="flex mobile:grid mobile:grid-cols-3 mobile:gap-1 p-4 mobile:p-2 w-[min(456px,100%)] self-center items-center">
          <Row className="items-center mobile:justify-self-center w-16 mobile:w-8 flex-shrink-0">
            <Col className="gap-1 grow -mt-4 mobile:items-center">
              <CoinAvatar token={coin} size={'lg'} />
              <div className="font-medium text-sm text-color ml-3.5">{coin?.symbol ?? '--'}</div>
            </Col>
          ...
          <Col className="items-end mobile:items-center mobile:justify-self-center mobile:ml-0 grow">
            ...
            <div className="text-lg font-medium text-color whitespace-nowrap">
              {'$' +
          ...
          <Col className="items-start mobile:items-center mobile:justify-self-center ml-8  mobile:ml-0 w-8">
            ...
            <div
              className={`text-lg font-medium ${isPositive ? 'text-green-400' : isNegative ? 'text-red-700' : 'text-green-400'
                }`}
            >
...
function KLineChartItemThumbnail({
  ...
  const lineColor = isPositive ? 'rgb(22 163 74)' : isNegative ? 'rgb(220 38 38)' : 'rgb(22 163 74)'
