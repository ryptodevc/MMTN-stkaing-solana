...
export default function SettingsPage() {
  return (
    <PageLayout mobileBarTitle="Settings" metaTitle="Settings - Magic Mittens">
        ...
        <InputBox
          ...
          suffix={
            <div
              ...
              className="text-color cursor-pointer clickable"
            >
              ↻ reset
            </div>
          }
  ...
  return (
    <Grid className="grid-cols-[12em,32em] mobile:grid-cols-1 gap-8 mobile:gap-1">
      <Row className="justify-self-end mobile:justify-self-start items-center">
        <div className="text-lg mobile:text-sm text-color">{name}</div>
        {tooltip && (
          <Tooltip panelClassName="bg-[#3b4146]" arrowClassName="bg-[#3b4146]">
            <Icon size="sm" heroIconName="question-mark-circle" className="mx-1 cursor-help text-color" />
            <Tooltip.Panel>
              <p className="w-80">{tooltip}</p>
            </Tooltip.Panel>
          </Tooltip>
        )}
        <div className="text-lg mobile:hidden text-color">: </div>
      </Row>
