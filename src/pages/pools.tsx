...
export default function PoolsPage() {
  ...
  return (
    <PageLayout contentButtonPaddingShorter mobileBarTitle="Pools" metaTitle="Pools - Magic Mittens">
      <PoolHeader />
      <PoolCard />
    </PageLayout>
  )
  ...
  return isMobile ? (
    showTvlVolume24h ? (
      <Row className="mx-auto my-2 text-base mobile:text-xs justify-self-start self-end text-[#abc4ff80] gap-4">
        <div className="whitespace-nowrap">
          TVL: <span className="font-medium text-color">${formatNumber(tvl)}</span>
        </div>
        <div className="whitespace-nowrap">
          Volume24H: <span className="font-medium text-color">${formatNumber(volume24h)}</span>
        </div>
      </Row>
    ) : null
  ) : (
    <Grid className="grid-cols-[1fr,1fr] mobile:grid-cols-2 grid-flow-row-dense items-center gap-y-8 pb-8">
      <Row className="justify-self-start gap-8">
        <div className="text-2xl mobile:text-lg text-color font-semibold">Pools</div>
        {showTvlVolume24h && (
      ...
      >
        <Icon heroIconName="plus-circle" className="text-color" size="sm" />
        <span className="text-color font-medium text-sm mobile:text-xs">Create Pool</span>
      </Row>
    </Grid>
  )
}
...
function PoolStakedOnlyBlock() {
  ...
  return (
    <Row className="justify-self-end mobile:justify-self-auto items-center">
      <span className="text-[rgb(255,255,255)] font-medium text-sm mobile:text-xs whitespace-nowrap">
        My Liquidity
      </span>
      <Switcher
        className="ml-2"
  ...
  return (
    <>
      <Popover placement="bottom-right">
        <Popover.Button>
          <div className={twMerge('mx-1 rounded-full p-2 text-color clickable justify-self-start', className)}>
        ...
        <Popover.Panel>
          <div>
            <Card
              className="flex flex-col py-3 px-4  max-h-[80vh] border-1.5 border-gray bg-light-card-bg shadow-cyberpunk-card"
              size="lg"
            >
...
function PoolSearchBlock({ className }: { className?: string }) {
  ...
  return (
    <Input
      ...
      className={twMerge(
        'px-2 py-2 mobile:py-1 gap-2 ring-inset ring-1 ring-[rgb(255,255,255)] rounded-xl mobile:rounded-lg pc:w-[12vw] mobile:w-auto',
        className
      )}
      inputClassName="font-medium text-sm mobile:text-xs text-[rgb(255,255,255)] placeholder-[rgb(255,255,255)]"
      prefix={<Icon heroIconName="search" size={isMobile ? 'sm' : 'smi'} className="text-[rgb(255,255,255)]" />}
      suffix={
        <Icon
          heroIconName="x"
          size={isMobile ? 'xs' : 'sm'}
          className={`text-[rgb(255,255,255)] transition clickable ${storeSearchText ? 'opacity-100' : 'opacity-0 pointer-events-none'
            }`}
          onClick={() => {
  ...
  return (
    <div className={className}>
      <div className="flex items-end">
        <div className="font-medium text-xl mobile:text-base text-color">Liquidity Pools</div>
        {showCount && (
        ...
        <Popover.Button>
          <div className={twMerge('rounded-full px-1 text-[#abc4ff80] clickable justify-self-start')}>
            <Icon
              ...
              className={filter.max || filter.min ? 'text-color' : 'text-[#abc4ff40]'}
            />
          </div>
        </Popover.Button>
        <Popover.Panel>
          <div>
            <Card
              className="flex flex-col py-3 px-4  max-h-[80vh] border-1.5 border-gray bg-light-card-bg shadow-cyberpunk-card items-center"
              size="lg"
            >
              <Grid className="grid-cols-1 items-center gap-2 ">
                <DecimalInput
                  className={`px-3 py-2 mobile:py-1 ring-inset ring-1 ${filterValueUnavailable ? 'ring-[rgba(255,86,86,0.94)]' : 'ring-[rgb(255,255,255)]'
                  ...
                  inputClassName="font-medium text-sm mobile:text-xs text-[rgba(196,214,255)] placeholder-[rgb(255,255,255)]"
                />
                <DecimalInput
                  className={`px-3 py-2 mobile:py-1 ring-inset ring-1 ${filterValueUnavailable ? 'ring-[rgba(255,86,86,0.94)]' : 'ring-[rgb(255,255,255)]'
                    } rounded-xl mobile:rounded-lg pc:w-[140px] mobile:w-auto`}
    ...
    if (isMobile) {
      return (
        <Row className={twMerge('items-center', className)}>
          <span className="text-[rgb(255,255,255)] font-medium text-sm mobile:text-xs">Refresh Pools</span>
          <RefreshCircle
            refreshKey="pools"
            freshFunction={() => {
              refreshPools()
...
function PoolCard() {
  ...
  const TableHeaderBlock = useMemo(
    () => (
      <Row
        type="grid-x"
        className="mb-3 h-12 justify-between sticky -top-6 backdrop-filter z-10 backdrop-blur-md bg-[rgba(20,27,35,0.2)] mr-scrollbar rounded-xl mobile:rounded-lg gap-2 grid-cols-[auto,1.6fr,1fr,1fr,1fr,.8fr,auto]"
      >
        ...
        <Grid className="grid-cols-[.4fr,1.2fr] clickable clickable-filter-effect no-clicable-transform-effect">
          ...
          <Row
            className="font-medium text-color text-sm items-center cursor-pointer"
            onClick={() => {
              setSortConfig({
                key: 'name',
        ...
        <Row
          className="font-medium text-color text-sm items-center cursor-pointer clickable clickable-filter-effect no-clicable-transform-effect"
          onClick={() => {
...
function PoolCardDatabaseBody({
  ...
  return !loading && (searchedItemsLength === 0 || (searchedItemsLength !== 0 && sortedItems?.length === 0)) ? (
    ...
    <List
      ...
      className="gap-3 mobile:gap-2 text-color flex-1 -mx-2 px-2" /* let scrollbar have some space */
    >
      {(info) => (
        <Collapse
...
function PoolCardDatabaseBodyCollapseItemFace({
  ...
  const pcCotent = (
    <Row
      type="grid-x"
      className={`py-5 mobile:py-4 mobile:px-5 bg-field items-center gap-2 grid-cols-[auto,1.6fr,1fr,1fr,1fr,.8fr,auto] mobile:grid-cols-[1fr,1fr,1fr,auto] rounded-t-3xl mobile:rounded-t-lg ${open ? '' : 'rounded-b-3xl mobile:rounded-b-lg'
        } transition-all`}
    >
  ...
  const mobileContent = (
    <Collapse open={open}>
      <Collapse.Face>
        <Row
          type="grid-x"
          className={`py-3 px-3 items-center gap-2 grid-cols-[auto,1.5fr,1fr,1fr,auto] bg-field mobile:rounded-t-lg ${open ? '' : 'rounded-b-3xl mobile:rounded-b-lg'
            }`}
        >
      ...
      <Collapse.Body>
        <Row
          type="grid-x"
          className="py-4 px-5 pl-12 relative items-center gap-2 grid-cols-[1.5fr,1fr,1fr,auto]  bg-field"
        >
...
function PoolCardDatabaseBodyCollapseItemContent({ poolInfo: info }: { poolInfo: HydratedPairItemInfo }) {
  const isMobile = useAppSettings((s) => s.isMobile)
  const balances = useWallet((s) => s.balances)
  const lightBoardClass = 'bg-[rgba(20,27,35,.2)]'
  ...
  return (
    ...
    >
      ...
      >
        <Row>
          <div className="flex-grow">
            <div className="text-gray-600 font-medium text-sm mobile:text-2xs mb-1">Your Liquidity</div>
            ...
            <div className="text-gray-600 font-medium text-sm mobile:text-2xs">
        ...
        <Row>
          <div className="flex-grow">
            <div className="text-gray-600 font-medium text-sm mobile:text-2xs mb-1">Assets Pooled</div>
            <div className="text-color font-medium text-base mobile:text-xs">
        ...
        <Row>
          <div className="flex-grow">
            <div className="text-gray-600 font-medium text-sm mobile:text-2xs mb-1">Your Share</div>
            <div className="text-color font-medium text-base mobile:text-xs">
      ...
      <Row
        className={`px-8 py-2 gap-3 items-center self-center justify-center ${isMobile ? lightBoardClass : ''
          } mobile:w-full`}
      >
        {isMobile ? (
          <Row className="gap-5">
            <Icon
              size="sm"
              heroIconName="plus"
              className="grid place-items-center w-10 h-10 mobile:w-8 mobile:h-8 ring-inset ring-1 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-xl mobile:rounded-lg text-color-secondary clickable clickable-filter-effect"
              onClick={() => {
          ...
          <>
            ...
            <Tooltip>
              <Icon
                ...
                className={`grid place-items-center w-10 h-10 mobile:w-8 mobile:h-8 ring-inset ring-1 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-xl mobile:rounded-lg text-color-secondary clickable-filter-effect ${correspondingFarm ? 'clickable' : 'not-clickable'
                  }`}
                onClick={() => {
                  routeTo('/farms', {
            ...
            <Tooltip>
              <Icon
                size="smi"
                iconSrc="/icons/pools-remove-liquidity-entry.svg"
                className={`grid place-items-center w-10 h-10 mobile:w-8 mobile:h-8 ring-inset ring-1 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-xl mobile:rounded-lg text-color-secondary ${hasLp ? 'opacity-100 clickable clickable-filter-effect' : 'opacity-50 not-clickable'
                  }`}
                onClick={() => {
                  hasLp &&
                    routeTo('/liquidity/add', {
                    ...
                    <div className="max-w-[300px] space-y-1.5">
                      <div className="flex gap-2 text-sm">
                        ...
                        <AddressItem
                          ...
                          canCopy
                          canExternalLink
                          textClassName="flex text-xs text-color justify-start "
...
function TextInfoItem({ name, value }: { name: string; value?: any }) {
  ...
  return isMobile ? (
    <div>
      <div className="mb-1 text-gray-600 font-medium text-2xs">{name}</div>
      ...
      {({ close }) => (
        ...
        >
          <Row className="justify-between items-center mb-6">
            <div className="text-3xl font-semibold text-color">Update Token Symbol/Name</div>
