...
import { MMTNTokenAddress } from '@/types/constants'
...
export async function getMMTNPriceFromLP(lpPoolID: string): Promise<number | undefined> {
  ...
  const MMTNPrice = quote * quotePrice / base;
  return MMTNPrice;
...
export async function getLpPrice(lpPoolID: string): Promise<number | undefined> {
  ...
  basePrice = quote * quotePrice / base;
  ...
  if (lpUiSupply == 0 || !lpUiSupply) {
    ...
    const lpPrice = (base * basePrice + quote * quotePrice) / (lpUiSupply + 59088209.226399);
    return lpPrice;
  }
}