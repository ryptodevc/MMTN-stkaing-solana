...
import { MMTNMint } from '@/application/token/wellknownToken.config'
...
export default function useAggregatorInitCoinFiller() {
  ...
  useEffect(() => {
    ...
    if (!coin2 && coin1?.mintString !== toPubString(MMTNMint) && !hasOutputCurrency) {
      useAggregator.setState({ coin2: getToken(MMTNMint) })
    }
