...
export default function CoinInputBox({
  ...
  return (
    <Row
      className={twMerge(
        `relative flex-col border-solid border-1 border-[#494949] bg-field bg-[#494949] cursor-text rounded-xl py-3 px-6 mobile:px-4 ${disabled && !noDisableStyle ? 'pointer-events-none-entirely cursor-default opacity-50' : ''
        }`,
        className
      )}
      style={style}
    ...
    >
      ...
      <Row className="justify-between mb-2">
        <div className="text-xl mobile:text-lg text-color" title={HTMLTitleTooltip}>
          {topLeftLabel}
        </div>
        <div
          className={`text-xl mobile:text-lg justify-self-end text-color ${disabledInput ? '' : 'clickable no-clicable-transform-effect clickable-filter-effect'
            }`}
          onClick={() => {
      ...
      <Row className="col-span-full items-center">
          ...
          <>
            ...
            >
              ...
              <Col className="items-start">
                <div
                  className={`text-color max-w-[7em] ${token ? 'min-w-[2em]' : ''
                    } overflow-hidden text-ellipsis font-medium text-xl flex-grow mobile:text-lg whitespace-nowrap`}
                >
                  {token?.symbol ?? '--'}
                </div>
              ...
              {showTokenSelectIcon && !disabledTokenSelect && (
                <Icon size="xs" heroIconName="chevron-down" className="text-color" />
              )}
            </Row>
            {/* divider */}
            <div className="my-1 mx-4 mobile:my-0 mobile:mx-2 border-r border-[rgba(142,134,155)] self-stretch" />
          </>
        )}
        <Row className="justify-between flex-grow-2">
          <Row className="gap-px items-center mr-2">
            {!hideMaxButton && (
              <Button
                ...
                className="py-0.5 px-1.5 rounded text-color bg-max bg-opacity-80 text-xl mobile:text-lg transition"
                onClick={() => {
                  if (onCustomMax) {
                    onCustomMax()
                    setInputedAmount(typeof maxValue === 'string' ? maxValue : toString(maxValue))
            ...
            {haveHalfButton && (
              <Button
                disabled={disabledInput}
                className="py-0.5 px-1.5 rounded text-color font-bold bg-min bg-opacity-80 text-xl mobile:text-lg transition"
                onClick={() => {
          ...
          <DecimalInput
            className="font-medium text-2xl text-color flex-grow w-full"
            disabled={disabledInput}
            ...
            onEnter={onEnter}
            inputClassName="text-right mobile:text-lg font-medium text-color"
            onBlur={(input) => {
              isOutsideValueLocked.current = false
      ...
      <Row className="items-center gap-2 justify-end text-xl mobile:text-lg text-color ">
        {/* token 2022 fee */}
        {transferFeeInfo && (
