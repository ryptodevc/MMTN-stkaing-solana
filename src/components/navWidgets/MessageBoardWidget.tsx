...
function MessageItem({
  ...
  return (
    <Col
      className="py-4 border-gray cursor-pointer clickable clickable-filter-effect"
      onClick={onClick}
    >
      <Row className="gap-4 items-center">
        <div className={`text-color ${haveReaded ? 'opacity-40' : 'opacity-80'} font-semibold`}>{item.title}</div>
        <Icon
          size="sm"
          className={`text-color ${haveReaded ? 'opacity-40' : 'hidden'}`}
    ...
    <>
      <PageLayoutPopoverDrawer
        ...
        renderPopoverContent={({ close: closePanel }) => (
          <div>
            <div className="pt-3 -mb-1 mobile:mb-2 px-6 text-gray-600 text-xs mobile:text-sm">
      ...
      >
        <Icon
          ...
          heroIconName="bell"
          className="text-color opacity-60 hover:opacity-75 clickable clickable-filter-effect clickable-mask-offset-3"
        />
      </PageLayoutPopoverDrawer>
      ...
      >
        {({ close }) => (
          <Card
            className="flex flex-col backdrop-filter backdrop-blur-xl p-8 mobile:py-4 w-[min(750px,100vw)] mobile:w-screen max-h-[min(850px,100vh)] mobile:h-screen border-1.5 border-gray bg-light-card-bg shadow-cyberpunk-card"
            size="lg"
          >
            <Row className="justify-between items-center mb-6">
              <div className="text-3xl font-semibold text-color">{currentMessageBoardItem?.title}</div>
