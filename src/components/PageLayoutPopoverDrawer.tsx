...
function SmartBubblePanel({
  ...
  const inlineSVG = useMemo(() => {
    ...
    return arrowPosition === 'left'
      ? `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg'><rect rx='20' x='8' width='calc(100% - 9px)' height='100%'/>${arrowTop && isArrowVisiable ? `<polygon points='8,${arrowTop - 8} 0,${arrowTop} 8,${arrowTop + 8}' />` : ''
      }</svg>")`
      : arrowPosition === 'top'
        ? `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg'><rect rx='20' x='0' y='8' width='100%' height='calc(100% - 9px)'/>${arrowLeft && isArrowVisiable
