...
export default function Tabs<T extends string = string>({
  ...
  return (
    <RadioGroup
      {...restProps}
      currentValue={restProps.currentValue}
      className={twMerge('rounded-full p-1', $transparentBg ? 'bg-transparent' : 'btn-small', className)}
      itemClassName={(checked) =>
        twMerge(
          ...
          `grid rounded-full place-items-center font-bold whitespace-nowrap ${checked ? 'text-color' : 'text-color'
          }`,
