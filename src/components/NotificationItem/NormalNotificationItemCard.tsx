...
export function NormalNotificationItemCard({ info, close }: { info: NormalNotificationItemInfo; close: () => void }) {
  ...
  console.log('log->description', description, description?.toString().includes('Wallet: '))
  return (
    <Card
      domRef={itemRef}
      className={`min-w-[260px] relative rounded-xl ring-1.5 ring-inset ${colors[type].ring} bg-field py-5 pl-5 pr-10 mx-4 my-4 overflow-hidden pointer-events-auto`}
    >
      {/* timeline */}
      ...
      <Icon
        size="smi"
        heroIconName="x"
        className="absolute right-3 top-3 clickable text-gray-600 opacity-50 mobile:opacity-100 hover:opacity-100 transition-opacity"
        onClick={() => {
      ...
      <Row className="gap-3">
        ...
        <div>
          ...
          {description && (
            description?.toString().includes('Wallet: ') ? 
            <div>
              <div className="font-medium text-sm whitespace-pre-wrap mobile:text-xs text-gray-600">
                Wallet:
              </div>
              <div className="font-medium text-sm break-words mobile:text-xs text-gray-600"
                style={{maxWidth:'300px'}}
              >
                {description.toString().substring(8)}
              </div>
            </div>
            :
            <div className="font-medium text-sm whitespace-pre-wrap mobile:text-xs text-gray-600">
