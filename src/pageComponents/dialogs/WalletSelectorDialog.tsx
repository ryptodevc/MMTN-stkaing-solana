...
function WalletSelectorPanelItem({
  ...
  return (
    <Row
      className={`relative items-center gap-3 m-auto px-6 mobile:px-3 mobile:py-1.5 py-3 w-64 mobile:w-[42vw] h-14  mobile:h-12 frosted-glass frosted-glass-teal rounded-xl mobile:rounded-lg ${detected ? 'opacity-100' : 'opacity-60'
    ...
    >
      <Icon className="shrink-0 icon-shadow" size={isMobile ? 'md' : 'lg'} iconSrc={wallet.adapter.icon} />
      <Row className="grow items-center justify-between flex-wrap">
        <div className="mobile:text-md text-2xl">{wallet.adapter.name}</div>
        {detected && !isMobile && (
          <Badge
            className={` mobile:text-2xs text-color ${showBadge ? 'opacity-80' : 'hidden'
...
function SimulateWallet({ onClick }: { onClick?(): void }) {
  ...
  return (
    <Col className="p-6 mobile:py-3 mobile:px-4 flex-grow ring-inset ring-1.5 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-3xl mobile:rounded-xl items-center gap-3 m-8 mt-2 mb-4">
      <div className="mobile:text-lg text-2xl text-color">Simulate Wallet Address</div>
      <Input
        className="w-full"
      ...
      />
      <Button
        className="frosted-glass-teal mobile:text-lg text-2xl"
        onClick={() => {
          if (valueRef.current) {
            // @ts-expect-error force
...
function PanelContent({
  ...
  return (
    <Card
      className="flex flex-col max-h-screen  w-[586px] mobile:w-screen rounded-3xl mobile:rounded-none border-1.5 border-gray overflow-hidden bg-light-card-bg shadow-cyberpunk-card"
      size="lg"
    >
      <Row className="items-center justify-between p-8 mobile:p-4">
        <div className="text-3xl mobile:text-2xl text-yellow-400">Connect your wallet</div>
        <Icon className="text-color cursor-pointer" heroIconName="x" onClick={close} />
      </Row>

      {/* Disclaimer */}
      <div className="grow text-xl mobile:text-xl leading-normal text-yellow-400 rounded p-4 mobile:p-2 mobile:px-4 mb-6 mobile:mb-4 mx-8 mobile:mx-6 bg-field">
        By connecting your wallet, you acknowledge that you have read, understand and accept the terms in the
        <Link href="#" className="text-color px-1.5" onClick={close}>
          Disclaimer
        </Link>
      </div>
      ...
      <Grid
        className={`px-8 mobile:px-6 gap-x-6 gap-y-3 mobile:gap-2 ${installedWallets.length === 1 ? 'grid-cols-1' : 'grid-cols-2'
          } grow`}
      >
      ...
      <div className={`flex-1 ${isAllWalletShown ? 'mt-4' : ''} h-32 overflow-auto no-native-scrollbar`}>
        <FadeInStable show={isAllWalletShown}>
          <div className="pt-8 h-full">
            <div className="text-yellow-600 text-xl mobile:text-xs px-8 py-1">OTHER WALLETS</div>
            <Grid
      ...
      <Row
        className="m-4 text-yellow-400 justify-center items-center clickable"
        onClick={() => setIsAllWalletShown((b) => !b)}
      >
        <div className="text-xl mobile:text-lg">Show uninstalled wallets</div>
        <Icon className="mx-2" size="sm" heroIconName={isAllWalletShown ? 'chevron-up' : 'chevron-down'}></Icon>
      </Row>

      <div className="py-4 text-color text-center font-medium text-xl border-t-1.5 border-gray">
        New here?{' '}
        <Link href="#" className="text-color">
          Get started on Magic Mittens Staking Dapp
        </Link>
      </div>
    </Card>
  )
