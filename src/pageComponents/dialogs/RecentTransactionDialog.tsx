...
function SingleRecentTransactionItem({ txInfo }: { txInfo: TxHistoryInfo }) {
  ...
  return (
    <LinkExplorer hrefDetail={`tx/${txInfo.txid}`} noTextStyle>
      <Row
        type="grid-x"
        className="group gap-[min(3.5vw,24px)] grid-cols-[1.2fr,2fr,1.2fr] rounded-lg py-4 px-4 hover:bg-[#04133d80] items-center"
      >
        {/* table head column: Transaction type */}
        <Row className="group-hover:underline underline-offset-1 items-center font-medium text-color text-xs gap-2">
          <Icon
        ...
        <div className="font-medium text-color text-xs">{txInfo.description}</div>
        {/* table head column: Date and time */}
        <div className="font-medium text-color text-xs">{toUTC(txInfo.time)}</div>
      </Row>
    </LinkExplorer>
  )
}
...
function MultiTransactionGroupItems({ txInfoGroup }: { txInfoGroup: TxHistoryInfo[] }) {
  ...
  const isMobile = useAppSettings((s) => s.isMobile)
  return (
    <div className="rounded-lg hover:bg-[#04133d80] ">
      <Collapse>
        <Collapse.Face>
          {({ isOpen }) => (
            <Row
            ...
            >
              {/* table head column: Transaction type */}
              <Row className="items-center font-medium text-color text-xs gap-2">
                <Icon
                  size={isMobile ? 'sm' : 'smi'}
                ...
                <div className="flex-none ml-0.5 grid place-items-center h-4 w-4 rounded-full bg-[#abc4ff40] text-color">
                  <Icon size="xs" heroIconName={isOpen ? 'chevron-up' : 'chevron-down'} />
                </div>
              </Row>
              {/* table head column: Details */}
              <div className="font-medium text-color text-xs">{headTx.description}</div>
              {/* table head column: Date and time */}
              <div className="font-medium text-color text-xs">{toUTC(headTx.time)}</div>
            </Row>
          )}
        </Collapse.Face>
        <Collapse.Body>
...
function PanelContent({ historyItems, onClose }: { historyItems: TxHistoryInfo[]; onClose(): void }) {
  ...
  return (
    <Card
      className="flex flex-col max-h-[60vh] mobile:max-h-screen rounded-3xl mobile:rounded-none w-[min(750px,100vw)] mobile:w-full border-1.5 border-gray overflow-hidden bg-light-card-bg shadow-cyberpunk-card"
      size="lg"
    >
      <Row className="justify-between items-center p-8">
        <div className="text-3xl text-yellow-400">Recent transactions</div>
        <Icon className="text-color cursor-pointer" heroIconName="x" onClick={onClose} />
      </Row>

      <Row
        type="grid-x"
        className="gap-[min(3.5vw,24px)] grid-cols-[1.2fr,2fr,1.2fr] pb-3 px-8 border-b-1.5 border-gray"
      >
        {/* table head column: Transaction type */}
        <div className="font-medium text-yellow-500 text-xl">Transaction type</div>
        {/* table head column: Details */}
        <div className="font-medium text-yellow-500 text-xl">Details</div>
        {/* table head column: Date and time */}
        <div className="font-medium text-yellow-500 text-xl">Date and time</div>
      </Row>
      ...
      <div className="overflow-y-auto flex-1 mx-4 my-2" /* let scrollbar have some space */>
        {historyItems.length > 0 ? (
          ...
          <div className="font-medium text-[rgba(165,146,72)] text-xl py-4 text-center">
            No recent transactions on Raydium
          </div>
        )}
      </div>

      <Row className="border-t-1.5 border-gray">
        <LinkExplorer
          className="py-4 rounded-none flex-grow font-medium text-color text-xl flex justify-center gap-1 items-center"
